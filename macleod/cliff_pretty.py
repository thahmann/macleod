...
TPTP_SUBSTITUTIONS = [
    TPTP_UNARY_SUBSTITUTIONS,
    TPTP_NARY_SUBSTITUTIONS,
    TPTP_BINARY_SUBSTITUTIONS,
    TPTP_QUANTIFIER_SUBSTITUTIONS]
...
LADR_SUBSTITUTIONS = [
    LADR_UNARY_SUBSTITUTIONS,
    LADR_NARY_SUBSTITUTIONS,
    LADR_BINARY_SUBSTITUTIONS,
    LADR_QUANTIFIER_SUBSTITUTIONS]
            ...
            if within_section and end_symbol not in line:
                ...
                if len(lines) > 0:
                    ...
                    raise ClifParsingError(
                        'Syntax error in clif input: no matching' +
                        end_symbol +
                        ' for ' +
                        begin_symbol +
                        ' on line ' +
            ...
            if search_end:
                ...
                if len(line) > 1:
                    ...
                    if len(line) < 2:
                        output.append(start)
                        raise ClifParsingError(
                            'Syntax error in clif input: no closing parenthesis found for ' +
                            CLIF_COMMENT +
                            ' on line ' +
                            str(
                                len(output) +
                    ...
                    if len(line[0].strip()) > 0:
                        output.append(start)
                        raise ClifParsingError(
                            'Syntax error in clif input: found illegal characters before closing parenthesis in ' +
                            CLIF_COMMENT +
                            ' on line ' +
                            str(
                                len(output) +
                    ...
                    if len(lines) > 0:
                        ...
                        output.append(start)
                        raise ClifParsingError(
                            'Syntax error in clif input: missing closing quotes for ' +
                            CLIF_COMMENT +
                            ' on line ' +
                            str(
                                len(output) +
                ...
                if len(parts2) < 2:
                    output.append(start)
                    raise ClifParsingError(
                        'Syntax error in clif input: no opening parenthesis found before ' +
                        CLIF_COMMENT +
                        ' on line ' +
                        str(
                            len(output) +
                ...
                if len(line[0].strip()) > 0:
                    output.append(start)
                    raise ClifParsingError(
                        'Syntax error in clif input: found illegal characters after ' +
                        CLIF_COMMENT +
                        ' on line ' +
                        str(
                            len(output) +
...
def get_nonlogical_symbols_and_variables(sentence):
    ...
    return (symbols - set(CLIF_LOGICAL_CONNECTIVES) -
            set(CLIF_OTHER_SYMBOLS) - set(['']) - variables, variables)
        ...
        if isinstance(pieces, str):
            ...
            if found_symbol == symbol:
                if not arity:
                    ...
                    raise ClifParsingError("the symbol " +
                                           symbol +
...
def translate_sentences(input_file_names, language, axiom=True):
    ...
    for sentence in sentences:
        if (not sentence[0] == "%"):
            # print sentence
            (nonlogical_symbols,
             variables) = get_nonlogical_symbols_and_variables(sentence)
            # print sentence
...
def replace_logical_connectives(pieces, nonlogical_symbols, language):
    ...
    if len(pieces) == 1:
        raise ClifParsingError(
            "missing argument for logical connective or predicate '" +
            pieces[0])
    ...
    for connective in BINARY_SUBSTITUTIONS.keys():
        if connective == pieces[0]:
            if len(pieces) > 3:
                # ensure it is really used in a binary way
                raise ClifParsingError(
                    "wrong use of logical connective '" +
                    connective +
                    "' in term '" +
                    str(pieces) +
                    "'")
...
def sentence_to_tptp(sentence, variables, nonlogical_symbols,
                     sentence_number, axiom=True):
