...
TPTP_SUBSTITUTIONS = [TPTP_UNARY_SUBSTITUTIONS, TPTP_NARY_SUBSTITUTIONS, TPTP_BINARY_SUBSTITUTIONS, TPTP_QUANTIFIER_SUBSTITUTIONS] 
...
LADR_QUANTIFIER_SUBSTITUTIONS = {CLIF_UNIVERSAL:'all', CLIF_EXISTENTIAL:'exists'}
...
#    for i in range(0, 1):
	...
	while i<len(sentences):
		if (sentences[i][0]=="%"):
			...
			if language=="LADR":
				translation = sentence_to_ladr(sentences[i], variables_list[i], nonlogical_list[i], int((i+1)*math.pow(10,digits)))
			else:
...
def replace_logical_connectives (pieces, nonlogical_symbols, language):
	...
	if language=="LADR":
		...
		logging.getLogger(__name__).warn("Unknown target language for translation: " + language)
	...
	for quantifier in QUANTIFIER_SUBSTITUTIONS.keys():
		if quantifier==pieces[0]:
			...
			for var in pieces[1]:
				if language=="TPTP":
					# special format for TPTP output: ! [x1] : ? [x2] : ! [x3] : ? [x4] : ...
					sentence += QUANTIFIER_SUBSTITUTIONS[quantifier] + " [" + var + "] : "
				else:
					# format for LADR output: all [x1] exists [x2] all [x3] exists [x4]
					sentence += QUANTIFIER_SUBSTITUTIONS[quantifier] + " " + var + " "
			#print "QUANTIFIER REMAINDER: " + str(pieces[2])
			remainder = replace_logical_connectives(pieces[2], nonlogical_symbols, language)
	...
	# recursion otherwise
	for i in range(0,len(pieces)):
		replacement = replace_logical_connectives(pieces[i], nonlogical_symbols, language)
	...
	for connective in NARY_SUBSTITUTIONS.keys():
		if connective==pieces[0]:
			...
			for i in range(2,len(pieces)):
				sentence += " " + NARY_SUBSTITUTIONS[connective] + " " + pieces[i]
			sentence +=  ")"
			#print "NARY: " +  sentence
			return sentence
...
def replace_variables (variables_dict, sentence, variables):
	...
	if isinstance(sentence, str):
		...
		for i in range(0,len(sentence)):
			sentence[i] = replace_variables(variables_dict, sentence[i], variables)
		return sentence
...
def sentence_to_tptp (sentence, variables, nonlogical_symbols, sentence_number, axiom=True):
	...
	sentence = replace_logical_connectives(sentence, nonlogical_symbols, "TPTP")
...
def sentence_to_ladr (sentence, variables, nonlogical_symbols, sentence_number):
	...
	sentence = replace_variables(var_dict, sentence, variables)
	sentence = replace_logical_connectives(sentence, nonlogical_symbols, "LADR")
	#sentence = quantifiers_to_tptp(sentence)
	sentence = sentence.replace("'","")
